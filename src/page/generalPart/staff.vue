<template>
  <div>
    ...
    <el-row class="containt">
      <el-form :inline="true" :model="filters" size="small" class="filterBlock" :gutter="20">
        <el-row class="filterContent">
          <el-form-item label="板块">
            <el-select placeholder="请选择板块" v-model="pageRequest.bin_id">
              <el-option
                v-for="plate in allPlate"
                :key="plate.id"
          ...
          <el-form-item label="姓名">
            <el-input v-model.trim="pageRequest.name" size="small" clearable></el-input>
          </el-form-item>
          <el-form-item label="手机号">
            <el-input v-model.trim="pageRequest.mobile" size="small" clearable></el-input>
          </el-form-item>
        ...
        >
          ...
          <el-form-item label="板块">
            <el-select
              placeholder="请选择板块"
              size="small"
              v-model="dataForm.bin_id"
              @click="changeRoleList"
            >
              <el-option
                v-for="plate in allPlate"
                :key="plate.id"
          ...
          <el-form-item label="角色类型">
            <el-select placeholder="请选择角色" size="medium" v-model="dataForm.role_id">
              <el-option v-for="role in roleType" :key="role.id" :label="role.sm" :value="role.id"></el-option>
            </el-select>
          </el-form-item>
...
<script>
import { staffList } from "@/api/general";
  ...
  data() {
    ...
    return {
      ...
      columns: [
        ...
        { prop: "addtime", label: "申请时间", minWidth: 150 }
  ...
  methods: {
    ...
    findTreeData: function(data) {
      ...
      this.post(api, data, res => {
        if (res.body.Status == 0) {
          if (this.getUserInfo().bin_id == 1) {
            this.menuData = [
              { name: "总平台", id: "-1", Children: res.body.Data[1][0] },
              {
                name: "汽车服务",
                id: "-2",
              ...
              //   id:"-3",
              //   disabled: true,
              //   Children: res.body.Data[3][0]
              // },
              {
                name: "智慧社区",
                id: "-4",
                Children: res.body.Data[4][0]
              }
            ];
          } else {
    ...
    getRoleType(plate_id) {
      this.post(
        ...
        { plate_id: plate_id, Rows: 999, N: 1 },
        res => {
          if (res.body.Status == 0) {
