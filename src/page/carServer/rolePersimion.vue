      ...
      <el-form :inline="true" :model="filters" size="medium" class="filterBlock" :gutter="20">
        <el-row class="filterContent">
          ...
          <el-form-item label="角色类型">
            <el-select v-model="pageRequest.sm" placeholder="请输入角色类型" size="medium">
              <el-option
                v-for="(option,index) in changeRole"
                :label="option.sm"
                :value="option.sm"
                :key="index"
              ></el-option>
...
export default {
  ...
  methods: {
    ...
    findPage: function(data) {
      ...
      this.post("PubInterface/Role/QueryRoleList", this.pageRequest, res => {
        ...
        if (res.body.Status == 0) {
          ...
          this.changeRole = res.body.Data.Rows;
    ...
    findTreeData: function() {
      ...
      this.post("PubInterface/Menu/QueryMenuList", { plate_id: 2 }, res => {
        if (res.body.Status == 0) {
