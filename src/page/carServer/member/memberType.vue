          ...
          <el-form :model="dataForm" label-width="120px" :rules="dataFormRules" ref="dataForm" label-position="right">
            ...
            <el-form-item label="类型服务项目" v-if="dataForm.type">
              <el-table ref="serverAllTable" :data="getServerList" tooltip-effect="dark" border style="width: 100%" @selection-change="handleSelectionValue">
                ...
                <el-table-column label="卡金额(元)" v-if="dataForm.type == 1">
                  <template slot-scope="scope">
                    <el-input v-model='scope.row.val' placeholder="请输入卡金额(元)"></el-input>
                  </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit: function(params) {
      ...
      this.isAdd = false;
      this.api('CarServer/Card/QueryCardTypeDetail',{id:params.id},res=>{
        this.dataForm.id = res.Data.id;
        this.dataForm.mer_id = res.Data.mer_id;
        this.dataForm.name = res.Data.name;
        this.dataForm.Img = res.Data.Img;
        this.dataForm.fuxm_id = res.Data.fuxm_ids; 
      })
      // this.dataForm = {
      //   id: params.id,
    ...
    getServer() {
      ...
      return new Promise((resolve, reject) => {
        this.post("CarServer/ServerProject/QueryProjectTypeSelect", {}, res => {
          if (res.body.Status == 0) {
            this.getServerList = res.body.Data;
            console.log(this.getServerList)
            for (var i = 0; i < this.getServerList.length; i++) {
              Vue.set(this.getServerList[i], "val", "");
            }
            // this.dataForm.fuxm_ids = JSON.parse(this.dataForm.fuxm_ids)
            if (this.getServerList.length > 0 && !this.operation) {
              for (var i = 0; i < this.dataForm.fuxm_id.length; i++) {
                for (var j = 0; j < this.dataForm.fuxm_id.length; j++) {
                  if (this.getServerList[i].id == this.dataForm.fuxm_id[j].id) {
                    this.getServerList[i].val = this.dataForm.fuxm_id[
                      j
                    ].val;
                    this.serverIndex.push({id:this.getServerList[i].id,val:this.getServerList[i].val});
                  }
                }
              }
            }
    ...
    toggleSelection(rows) {
      if (rows) {
        rows.forEach(row => {
          this.$refs.serverAllTable.toggleRowSelection(row);
        });
      } else {
        this.$refs.serverAllTable.clearSelection();
      }
    },
    getMyServer() {
      this.$nextTick(function() {
        this.serverIndex.forEach(row => {
