      ...
      <el-row>
        ...
        >
          <el-row class="filterContent">
            ...
            <el-form-item label="项目大类型">
              <el-input
                v-model.trim="parameter.lx"
                size="medium"
                clearable
              ></el-input>
            </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { prop: "lx", label: "项目大类型", minWidth: 100 },
        { prop: "sm", label: "项目说明", minWidth: 100 }
      ]
    };
  },
  mounted() {
  ...
  methods: {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            let api;
            if (this.operation) {
              api = "CarServer/ServerProject/AddProjectType";
            } else {
              api = "CarServer/ServerProject/UpdateProjectType";
            }
    ...
    handleDelete(row) {
      this.$confirm("确认删除选中记录吗？", "提示", {
        ...
        this.post(
          "CarServer/ServerProject/DelProjectType",
          { id: row },
          res => {
            if (res.body.Status == 0) {
    ...
    getServerTypeList() {
      this.post("CarServer/ServerProject/QueryProjectTypeSelect", {}, res => {
        if (res.body.Status == 0) {
          this.serverTypeProject = res.body.Data;
          this.loading = false;
        } else if (res.body.Status == -1) {
          this.$notify.error({
            title: "登录失效",
