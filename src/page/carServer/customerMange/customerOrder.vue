      ...
      <el-row>
        <el-col :span="24">
          ...
          <el-table ref="multipleTable" border :data="tableList" tooltip-effect="dark" stripe style="width: 100%" size="medium" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="Img" label="客户签字" min-width="120" header-align="center" align="center">
              <!-- 图片的显示 -->
              <template slot-scope="scope">
                <img v-if="scope.row.khqz" :src="baseImgPath +scope.row.khqz" min-width="70" height="70" />
              </template>
            </el-table-column>
        ...
        <!--新增编辑界面-->
        <el-dialog title="客户服务详情" width="50%" :visible.sync="dialogVisible" :close-on-click-modal="false">
...
export default {
  ...
  methods: {
    ...
    handleServer: function(id) {
      this.dialogVisible = true;
      this.post("CarServer/Customer/QueryMerOrderDetailList", {dd_id:id}, res => {
        if (res.body.Status == 0) {
