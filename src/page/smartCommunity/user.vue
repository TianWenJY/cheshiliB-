<template>
  <div>
    ...
    <el-row class="containt">
      <el-form :inline="true" :model="filters" size="medium" class="filterBlock" :gutter="20">
        <el-row class="filterContent">
          <el-form-item label="所属社区" v-if="this.getUserInfo().bin_id == 1"  label-width="80px">
            <el-select placeholder="选择所属社区" filterable size="medium" v-model="pageRequest.p_id">
              <el-option label='全部' value="-1"></el-option>
              <el-option v-for="(commun,index) in Community" :label='commun.sqmc' :value="commun.u_id" :key="index"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="登陆类型"  label-width="80px">
            <el-select placeholder="选择登陆类型" size="medium" v-model="pageRequest.role_type">
              <el-option label='全部' value="-1"></el-option>
          ...
          <el-form-item> 
          <kt-button label="查询" type="primary" @click="findPage(null)" size="medium" :loading="onLoading"/>
      ...
      <!--表格内容栏-->
      <!-- @findPage="findPage" -->
      <kt-table :highlightCurrentRow="true" @findPage="findPage" :data="pageResult" :pageRequest="pageRequest" ref="table"
        :havePassWord="havePassWord" :columns="columns" :showBatchDelete="false" @handleCurrentChange="handleRoleSelectChange"
        @resPassword="resPassword" @handleEdit="handleEdit" @handleDelete="handleDelete">
      </kt-table>
      <!-- </el-col> -->
      <!--新增编辑界面-->
      <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
        <el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size">
          <el-form-item label="所属社区" v-if="this.getUserInfo().bin_id == 1">
            <el-select placeholder="选择所属社区" size="medium" v-model="dataForm.p_id">
              <el-option v-for="(commun,index) in Community" :label='commun.sqmc' :value="commun.u_id" :key="index"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="角色类型">
            <el-select placeholder="请选择角色" size="medium" v-model="dataForm.role_type">
              <el-option label="物业" value="0">
              </el-option>
          ...
          <el-form-item label="角色名称">
            <el-select placeholder="请选择角色名称" size="medium" v-model="dataForm.role_id">
      ...
      <div class="menu-container">
        ...
        <el-tree :data="menuData" size="mini" show-checkbox node-key="id" :props="defaultProps" style="width: 100%;pading-top:20px;"
          ref="menuTree" :render-content="renderContent" :check-strictly="false">
        </el-tree>
...
<script>
  import { userList } from "@/api/smart";
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        pageRequest: {
          ...
          role_id: -1,
          role_type: "-1",
          p_id: this.getUserInfo().bin_id == 1 ? "-1" : this.getUserInfo().p_id == 0 ? this.getUserInfo().mer_id : this.getUserInfo().p_id,
          name: "",
          mobile: "",
          addtime: "",
          N: 1,
    ...
    computed: {
      ...
      this_type() {
        return this.dataForm.role_type;
      }
    },
    methods: {
      ...
      handleAdd: function () {
        ...
        this.dataForm = {
          ...
          name: "",
          mobile: "",
          p_id: this.getUserInfo().bin_id == 1 ? "" : this.getUserInfo().p_id == 0 ? this.getUserInfo().mer_id : this.getUserInfo().p_id,
        };
      },
      // 显示编辑界面
      ...
      getRoleType(param) {
        this.post("PubInterface/Role/QueryRoleList", param, res => {
          if (res.body.Status == 0) {
    ...
    watch: {
      this_type(curVal, oldVal) {
        if (curVal == 0) {
          this.getRoleType({ plate_id: 4, role_type: curVal });
        } else if (curVal == 1) {
          this.getRoleType({ plate_id: 4, role_type: curVal });
        }
      }
