...
import { getList, submit } from "@/api/smart";
...
export default {
  ...
  data() {
    return {
      ...
      parkList: [],
      columns: [
        { prop: "sqmc", label: "所属社区", minWidth: 120 },
  ...
  computed: {
    getApi() {
      return this.pageRequest.mer_id;
    },
    getEdAdApi() {
      return this.dataForm.mer_id;
    }
  },
  methods: {
    // 获取分页数据
    ...
    findPage: function(data) {
      ...
      let api = "NoSense/BlackWhite/QueryWhiteList";
      let params;
      let thisApi;
    ...
    // 删除
    handleDelete(row) {
      let api = "NoSense/BlackWhite/DelWhite";
      let params;
      let thisApi;
      if (location.href.indexOf("csl.ycpicc.com") != -1) {
        thisApi = "http://csl.ycpicc.com:8058/zhsq/socket/post.action";
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            let api;
            if (this.operation) {
              api = "NoSense/BlackWhite/AddWhite";
            } else {
              api = "NoSense/BlackWhite/EditWhite";
            }
            let params;
            let thisApi;
            if (location.href.indexOf("csl.ycpicc.com") != -1) {
